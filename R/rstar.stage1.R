...
"rstar.stage1" <- function(log.output,
  ...
  #----------------------------------------------------------------------------#
  y.data <- cbind(
    100 * log.output[9:(t.end + 8)],
    inflation[9:(t.end + 8)]
  )
  x.data <- cbind(
    100 * log.output[8:(t.end + 7)],
    ...
    covid.dummy[7:(t.end + 6)]
  ) # covid dummy and two lags
  ...
  matrices <- unpack.parameters.stage1(theta,
    y.data = y.data, x.data = x.data,
