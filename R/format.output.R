...
# ------------------------------------------------------------------------------#
"format.output" <- function(estimation, real.rate, covid.dummy, 
                          start, end, run.se = TRUE, estimate.theta = TRUE) {
  # One-sided (filtered) estimates
  one.sided.est <- cbind(
    estimation$rstar.filtered,
  ...
  if (run.se) {
    ...
    output[13, sec2] <- "Draws excluded:"
    output[13, (sec2 + 2)] <- estimation$se$number.excluded
    output[13, (sec2 + 3)] <- "Total:"
    output[13, (sec2 + 4)] <- niter
    output[14, sec2] <- "Percent excluded:"
    output[14, (sec2 + 2)] <- as.numeric(output[13, (sec2 + 2)]) / (as.numeric(output[13, (sec2 + 2)]) + as.numeric(output[13, (sec2 + 4)]))
    output[15, sec2] <- "Draws excluded because a_r > -0.0025:"
    output[15, (sec2 + 4)] <- estimation$se$number.excluded.a.r
    output[16, sec2] <- "Draws excluded because b_y <  0.025:"
    output[16, (sec2 + 4)] <- estimation$se$number.excluded.b.y
    output[17, sec2] <- "Draws excluded because a_y1 + a_y2 < 1:"
    output[17, (sec2 + 4)] <- estimation$se$number.excluded.a1a2
  }

  # Signal-to-noise ratios
  output[19, sec2] <- "Signal-to-noise Ratios"
  output[20, sec2] <- "lambda_g"
  output[20, (sec2 + 1)] <- estimation$lambda.g
  output[21, sec2] <- "lambda_z"
  output[21, (sec2 + 1)] <- estimation$lambda.z
  output[19, (sec2 + 4)] <- "Log Likelihood"
  output[20, (sec2 + 4)] <- estimation$log.likelihood
